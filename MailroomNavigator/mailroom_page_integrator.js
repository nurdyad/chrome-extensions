...
const MAILROOM_INTEGRATOR_DEBUG = false;
const mailroomLog = (...args) => {
    if (MAILROOM_INTEGRATOR_DEBUG) console.log(...args);
};

if (!/\/mailroom\/(rejected|preparing)(?:[/?#]|$)/i.test(window.location.pathname)) {
    mailroomLog("Mailroom Integrator: skipped unsupported page.");
} else {
...
function extractDataFromMailroomRow(row) {
    mailroomLog("Mailroom Integrator: Processing row:", row);
    const cells = row.querySelectorAll('td');
    mailroomLog(`Mailroom Integrator: Row has ${cells.length} cells.`);

    // Based on the latest screenshot, expecting 7 columns (0-6)
    if (cells.length < 7) { 
        mailroomLog("Mailroom Integrator: Row has too few cells (<7), skipping.", row);
        return null;
    }
    ...
    mailroomLog("Mailroom Integrator: Extracted Document ID:", documentId);
    if (!documentId || !/^\d+$/.test(documentId)) { // Allow any number of digits
        mailroomLog("Mailroom Integrator: Document ID not found or invalid format.", documentId);
        return null;
    }
    ...
    mailroomLog("Mailroom Integrator: Extracted Status Content (col 1):", statusContent);

    // --- Column 2: Time Spent (e.g., "Paused") ---
    const timeSpent = cells[2]?.innerText.trim();
    mailroomLog("Mailroom Integrator: Extracted Time Spent (col 2):", timeSpent);
    ...
    mailroomLog("Mailroom Integrator: Extracted Job ID (UUID from col 3 title):", jobId);

    // --- Column 4: Expected Return Date ---
    const expectedReturnDate = cells[4]?.innerText.trim();
    mailroomLog("Mailroom Integrator: Extracted Expected Return Date (col 4):", expectedReturnDate);
    ...
    mailroomLog("Mailroom Integrator: Extracted Practice Name (col 5):", practiceName);
    ...
    mailroomLog("Mailroom Integrator: Extracted Urgent Status (col 6):", urgentStatus);
    ...
    mailroomLog("Mailroom Integrator: Final extracted data for row.");
    return { 
        documentId, 
        originalNameContent, 
        nhsNo, 
...
function addMailroomDocIdListeners() {
    mailroomLog("Mailroom Integrator: Running addMailroomDocIdListeners.");
    const tableRows = document.querySelectorAll('#documents-table-body tr'); 
    mailroomLog(`Mailroom Integrator: Found ${tableRows.length} table rows with #documents-table-body tr.`);
        ...
        if (docIdElement) {
            ...
            if (rowData) {
                ...
                docIdElement.addEventListener('dblclick', (e) => { 
                    ...
                    mailroomLog("Mailroom Integrator: Document ID DOUBLE-CLICKED! Sending message to background.", e.currentTarget.dataset);
                    chrome.runtime.sendMessage({
                        ...
                        mailroomLog('Mailroom Integrator: Message sent response from background:', response);
                    }).catch(error => {
                        console.error('Mailroom Integrator: Error sending message:', error);
                    });
                });
            } else {
                mailroomLog("Mailroom Integrator: Row data extraction failed for element, no listener added for:", docIdElement);
            }
        } else {
            mailroomLog("Mailroom Integrator: No Document ID span found in row (td:first-child div > span.grow-0.overflow-x-clip):", row);
        }
    });
}
